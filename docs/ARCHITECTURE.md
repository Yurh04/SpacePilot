...
5. `AppSnapshotProjection` 在 MainActor 外将快照转换为限制数量的界面投影，SwiftUI 只接收投影后的状态。
6. FSEvents 批量产生变化路径。`FileSystemChangeReconciler` 将受影响的目录统计标记为过期，并请求最小必要范围的后台刷新。
7. 用户选中应用后，可调用 `ApplicationDetailAnalyzer` 精确计算关联路径和大小；结果会缓存，不会在每次页面切换时重复计算。

## 应用所有权模型

关联候选项可能来自准确 Bundle ID、容器 ID、嵌入组件、签名/Team 信息、Spotlight 或版本化知识库。每条有效关联均记录：
